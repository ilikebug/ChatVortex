...
import { isMultimodalModel, getSupportedFileTypes } from "@/lib/api";
import {
  createImageData,
...
interface ChatInputProps {
  onSendMessage: (content: string, files?: File[], images?: any[]) => void;
  isLoading?: boolean;
  placeholder?: string;
...
export const ChatInput = forwardRef<ChatInputRef, ChatInputProps>(
  function ChatInput(
    ...
    const [uploadedImages, setUploadedImages] = useState<any[]>([]); // 图片数据
          ...
          recognition.onresult = (event) => {
            ...
            if (finalTranscript) {
              ...
              setMessage((prev) => {
                ...
                const cleanPrev = prev.replace(/\s*\[临时识别\].*$/, "");
                const newMessage =
                  cleanPrev + (cleanPrev ? " " : "") + processedText;
    ...
    // 检查当前模型是否支持多模态
    const supportsMultimodal = isMultimodalModel(currentModel);
    ...
    const handleFileSelect = async (
      ...
      const supportedTypes = getSupportedFileTypes(currentModel);
      const validFiles = files.filter((file) => {
        if (!supportedTypes.includes(file.type)) {
          alert(
            `不支持的文件格式: ${file.name}\n\n支持的格式: ${supportedTypes.join(", ")}`
          );
          return false;
        }
        ...
        <div className="relative">
          ...
          <div className="flex items-center gap-2 mb-2 relative">
            ...
            <motion.button
              ...
              title={
                supportsMultimodal ? "上传图片文件" : "图片上传功能暂时不可用"
              }
            >
              <Paperclip size={16} />
            </motion.button>
            ...
            {(isRecording || isListening) && (
              ...
              >
                <div className="bg-red-500 text-white px-3 py-1 rounded-full text-xs flex items-center gap-2 shadow-lg">
                  ...
                  {micLevel > 0 && (
                    <span className="text-xs opacity-80">🎵{micLevel}</span>
                  )}
          ...
          <form onSubmit={handleSubmit} className="relative">
            ...
            >
              ...
              {/* 输入区域 */}
              <div className="flex-1 flex flex-col gap-3 p-4 relative z-10">
                {/* 文件预览区域 */}
                {uploadedFiles.length > 0 && (
                  ...
                  >
                    {uploadedFiles.map((file, index) => {
                      const isImage = isSupportedImageType(file);
                ...
                <div className="flex items-end gap-3">
                  ...
                  <motion.button
                    ...
                    whileHover={{
                      scale:
                        message.trim() || uploadedFiles.length > 0 ? 1.05 : 1,
                      boxShadow:
                    ...
                    whileTap={{
                      scale:
                        message.trim() || uploadedFiles.length > 0 ? 0.95 : 1,
                    }}
                    className={cn(
